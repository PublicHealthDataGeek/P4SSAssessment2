...
plt.colorbar(orientation='horizontal', shrink=0.3, label="Number of rats caught per week", extend='max')
...
plt.colorbar(orientation='horizontal', shrink=0.3, label="Human population density", extend='max')
...
#save the deaths as a text file - doesnt seem to work ok
with open('ave_deaths.txt', "w") as f:
    writer = csv.writer(f)
    writer.writerow(average_deaths)	    
f.close() 
...
plt.colorbar(orientation='horizontal', shrink=0.45, label="Average deaths per week", extend='max')    
